 ...
 if (c.type === 'GRAVEYARD_COUNT') {
  ...
  if (c.type === 'BOARD_STATE') {
    switch (String(c.value)) {
      ...
      case 'ONLY_CREATURE_ON_BOARD': {
        const result = p.board.length === 1
        console.log('[ONLY_CREATURE_ON_BOARD] check', { boardLength: p.board.length, result })
        return result
      }
...
export function applyAction(
  ...
  const skipPreTarget = [
    ...
    EffectActionType.DAMAGE_ADJACENT,
    EffectActionType.ATTACK_SPELL,
    EffectActionType.DAMAGE_AND_DRAW_IF_KILL,
  ...
  switch (action.type as EffectActionType) {
    ...
    case EffectActionType.DAMAGE_ADJACENT: {
      const mainDamage = action.amount ?? 0
      const adjacentDamage = Number(action.value) ?? 0
      
      console.log('[DAMAGE_ADJACENT] start', { mainDamage, adjacentDamage, targetsHints })
      
      // Resolver objetivo principal
      const resolved = resolveSingleTarget(state, playerIndex, action.target, pickHint())
      
      console.log('[DAMAGE_ADJACENT] resolved target', resolved)
      
      if (resolved?.kind === 'CREATURE') {
        const targetPlayer = state.players[resolved.playerIndex]
        const targetIndex = resolved.index
        const target = targetPlayer.board[targetIndex]
        
        console.log('[DAMAGE_ADJACENT] target creature', { targetIndex, target, boardLength: targetPlayer.board.length })
        
        if (target) {
          // Daño al objetivo principal
          console.log('[DAMAGE_ADJACENT] applying main damage', { before: target.health, damage: mainDamage })
          if (hasAbility(target, Ability.ESCUDO)) {
            target.abilities = target.abilities.filter(a => a !== String(Ability.ESCUDO))
          } else {
            target.health -= mainDamage
            target.damagedThisTurn = true
          }
          console.log('[DAMAGE_ADJACENT] after main damage', { after: target.health })
          
          // Daño a criaturas adyacentes (índice-1 e índice+1)
          const adjacentIndices = [targetIndex - 1, targetIndex + 1]
          
          console.log('[DAMAGE_ADJACENT] checking adjacent creatures', { adjacentIndices, boardLength: targetPlayer.board.length })
          
          for (const adjIndex of adjacentIndices) {
            if (adjIndex >= 0 && adjIndex < targetPlayer.board.length) {
              const adjCreature = targetPlayer.board[adjIndex]
              
              console.log('[DAMAGE_ADJACENT] applying adjacent damage', { adjIndex, before: adjCreature.health, damage: adjacentDamage })
              
              if (hasAbility(adjCreature, Ability.ESCUDO)) {
                adjCreature.abilities = adjCreature.abilities.filter(a => a !== String(Ability.ESCUDO))
              } else {
                adjCreature.health -= adjacentDamage
                adjCreature.damagedThisTurn = true
              }
              
              console.log('[DAMAGE_ADJACENT] after adjacent damage', { adjIndex, after: adjCreature.health })
            }
          }
          
          console.log('[DAMAGE_ADJACENT] all damage applied, checking deaths')
          console.log('[DAMAGE_ADJACENT] board state', targetPlayer.board.map((c, i) => ({ index: i, cardId: c.cardId, health: c.health })))
          
          // Limpiar criaturas muertas
          const deadIndices: number[] = []
          for (let i = 0; i < targetPlayer.board.length; i++) {
            if (targetPlayer.board[i].health <= 0) {
              deadIndices.push(i)
            }
          }
          
          console.log('[DAMAGE_ADJACENT] creatures to remove', { deadIndices })
          
          // Eliminar de mayor a menor para no desplazar índices
          for (const idx of deadIndices.sort((a, b) => b - a)) {
            const [dead] = targetPlayer.board.splice(idx, 1)
            targetPlayer.graveyard.unshift(dead.cardId)
            notifyLeaveBattlefield(state, resolved.playerIndex, dead.id)
            notifyEffectTriggered(state, resolved.playerIndex, dead.cardId, 'ON_DEATH')
            console.log('[DAMAGE_ADJACENT] removed creature', { idx, cardId: dead.cardId })
          }
          
          console.log('[DAMAGE_ADJACENT] final board state', { 
            boardLength: targetPlayer.board.length,
            creatures: targetPlayer.board.map(c => ({ cardId: c.cardId, health: c.health }))
          })
        } else {
          console.log('[DAMAGE_ADJACENT] ERROR: target not found!')
        }
      } else {
        console.log('[DAMAGE_ADJACENT] ERROR: resolved is not a CREATURE', resolved)
      }
      break
    }
    ...
    case EffectActionType.SCRY: {
      ...
      // La UI se encargará de mover las cartas si el jugador elige "bottom"
      onScryRequest(state, {
        playerIndex: pIdx,
        cards: topCards
      })
      
      console.log('[SCRY] UI modal triggered, waiting for player input')
      
      break
    }

    case EffectActionType.ADVANCED_SELECTION: {
      ...
      // Obtener las N cartas del tope
      const selectionCount = Math.min(amount, p.deck.length)
      const topCards = p.deck.slice(0, selectionCount)
      console.log('[ADVANCED_SELECTION] showing top cards', { cards: topCards, count: selectionCount })
      
      // Activar el modal para que el jugador vea las cartas, elija 1 y ponga el resto al fondo
      // La UI se encargará de toda la manipulación del deck
      onAdvancedSelectionRequest(state, {
        playerIndex: pIdx,
        cards: topCards
      })
      
      console.log('[ADVANCED_SELECTION] UI modal triggered, waiting for player selection')
      
      break
    }

    case EffectActionType.DAMAGE_AND_DRAW_IF_KILL: {
      const damage = action.amount ?? 0
      const drawAmount = Number(action.value) ?? 1
      
      console.log('[DAMAGE_AND_DRAW_IF_KILL] start', { damage, drawAmount, targetsHints })
      
      // Resolver el objetivo (debe ser una criatura)
      const resolved = resolveSingleTarget(state, playerIndex, action.target, pickHint())
      
      console.log('[DAMAGE_AND_DRAW_IF_KILL] resolved target', resolved)
      ...
      if (resolved?.kind === 'CREATURE') {
        ...
        if (target) {
          ...
          console.log('[DAMAGE_AND_DRAW_IF_KILL] target before damage', { 
          ...
          if (hasAbility(target, Ability.ESCUDO)) {
            target.abilities = target.abilities.filter(a => a !== String(Ability.ESCUDO))
            console.log('[DAMAGE_AND_DRAW_IF_KILL] shield absorbed damage')
          } else {
            target.health -= damage
            target.damagedThisTurn = true
            console.log('[DAMAGE_AND_DRAW_IF_KILL] damage applied', { healthAfter: target.health })
          }
          
          // Detectar si el objetivo murió
          const targetDied = target.health <= 0 && healthBefore > 0
          console.log('[DAMAGE_AND_DRAW_IF_KILL] checking if target died', { targetDied })
          
          // Limpiar criatura muerta
          if (target.health <= 0) {
            const [dead] = targetPlayer.board.splice(targetIndex, 1)
            targetPlayer.graveyard.unshift(dead.cardId)
            notifyLeaveBattlefield(state, resolved.playerIndex, dead.id)
            notifyEffectTriggered(state, resolved.playerIndex, dead.cardId, 'ON_DEATH')
            console.log('[DAMAGE_AND_DRAW_IF_KILL] target died and removed from board', { cardId: dead.cardId })
          }
          
          // Si el objetivo murió, robar cartas
          if (targetDied) {
            const caster = state.players[playerIndex]
            console.log('[DAMAGE_AND_DRAW_IF_KILL] target was killed, drawing cards', { 
              drawAmount, 
              deckSize: caster.deck.length 
            })
            
            for (let i = 0; i < drawAmount; i++) {
              if (caster.deck.length > 0) {
                const drawn = caster.deck.shift()
                if (drawn) {
                  caster.hand.push(drawn)
                  console.log('[DAMAGE_AND_DRAW_IF_KILL] drew card', { card: drawn, handSize: caster.hand.length })
                }
              } else {
                console.log('[DAMAGE_AND_DRAW_IF_KILL] cannot draw - deck is empty')
              }
            }
          } else {
            console.log('[DAMAGE_AND_DRAW_IF_KILL] target survived, no cards drawn')
          }
        }
      }
...
case EffectActionType.DISCOVER_PAY_LIFE: {
  ...
  console.log('[DISCOVER_PAY_LIFE] targetsHints available:', targetsHints)
  ...
  if (choice === 'BUFF' && action.options?.buff) {
    ...
    // Pasar targetsHints solo si el sub-efecto lo necesita
    // Los hints disponibles se usarán por el sub-efecto según su propio target
  ...
  break
    ...
    case EffectActionType.GRANT_TEMP_DRAW_ON_KILL: {
      const amount = action.amount ?? 1
      const t = ensureTarget()
      if (!t) break
      ...
      if (t.kind === 'CREATURE') {
        ...
        if (cr) {
          ...
          console.log('[GRANT_TEMP_DRAW_ON_KILL] granted to creature', { 
            creatureId: cr.id, 
            cardId: cr.cardId, 
...
case EffectActionType.ATTACK_SPELL: {
  console.log('[ATTACK_SPELL] start', { targetsHints })
  ...
  if (hintAtk && hintDef) {
    ...
    console.warn('[ATTACK_SPELL] missing targets', { hasAttacker: !!hintAtk, hasDefender: !!hintDef })
  }
  break
}
...
case EffectActionType.DESTROY_TOP_HEALTH_CREATURES: {
  const count = action.amount ?? 3
  
  console.log('[DESTROY_TOP_HEALTH] start', { count, oppBoardSize: opp.board.length })
  
  // Ordenar criaturas enemigas por vida (de mayor a menor)
  ...
  // Tomar las primeras N criaturas (las que tienen más vida)
...
export function updateConditionalBuffs(state: GameState, playerIndex: number): void {
  ...
  for (let i = 0; i < p.board.length; i++) {
    ...
    if (isOnlyCreature && !hasConditionalBuff) {
      ...
      console.log('[updateConditionalBuffs] Applied buff', { newAttack: ent.attack, newHealth: ent.health })
    }
    ...
    if (!isOnlyCreature && hasConditionalBuff) {
      ...
      console.log('[updateConditionalBuffs] Removed buff', { newAttack: ent.attack, newHealth: ent.health })
    }
  }
}