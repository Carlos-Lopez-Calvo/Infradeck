...
export enum GamePhase {
  ...
  export interface CreatureOnBoard extends CardInZone {
      ...
      programmedEffects?: (EffectActionType | string)[] // <-- Allow custom string effects
    }
  ...
  triggeredByDamageFrom?: number // Qué jugador causó el daño letal
}
...
triggerPriority(state, GamePhase.START)
  ...
  // ✅ Aplicar bonus de Final Stand si corresponde
  applyFinalStandBonus(state, state.turn.currentPlayerIndex)
  
  // Resto de lógica de inicio de turno...
...
export function playCard(
  ...
  options?: PlayOptions
    ...
    if (card.type === CardType.CREATURE) {
        ...
        effects: [], // Add this property to satisfy CreatureOnBoard
        // programmedEffects is optional and can be omitted or set to []
        }
...
if (def && def.effects) {
  ...
  if (target.kind === 'HERO') {
    ...
    // ✅ Verificar inmunidad de Final Stand ANTES de aplicar daño
    if (hasFinalStandImmunity(state, targetPlayerIndex)) {
  console.log(`🛡️ Player ${targetPlayerIndex} is immune to damage (Final Stand)`)
  return // No daño
}
    
    const oldLife = p.life
    p.life = Math.max(0, p.life - amount)
    console.log(`💔 Player ${targetPlayerIndex} takes ${amount} damage (${oldLife} → ${p.life})`)
    
    // ✅ Verificar Final Stand SI el daño fue letal
    if (p.life <= 0 && oldLife > 0) {
      const finalStandActivated = checkAndActivateFinalStand(
        state, 
        targetPlayerIndex, 
        playerIndex // Quien causó el daño
      )
      
      if (finalStandActivated) {
        console.log(`⚡ Final Stand prevented death for Player ${targetPlayerIndex}`)
      }
    }
  }
...
function hasAbility(creature: CreatureOnBoard, ability: Ability) {
 ...
 // Líneas ~840-880: Corregir declareAttackHero también
...
export function declareAttackCreature(state: GameState, attackerIndex: number, attackerBoardIndex: number, defenderBoardIndex: number): AttackResult {
  ...
  function createTokenFromValue(value: string): CreatureOnBoard | undefined {
    ...
    if (value === 'TOKEN_2_2_PRISA') {
      return { id, cardId: 'TOKEN_2_2_PRISA', ownerId: '', attack: 2, health: 2, exhausted: false, abilities: [String(Ability.PRISA)], effects: [] }
    }
    if (value === 'TOKEN_4_4_PRISA_LIFESTEAL') {
      return { id, cardId: 'TOKEN_4_4_PRISA_LIFESTEAL', ownerId: '', attack: 4, health: 4, exhausted: false, abilities: [String(Ability.PRISA), String(Ability.ROBO_DE_VIDA)], effects: [] }
    }
...
// ✅ FUNCIONES DE HABILIDADES REALES (solo las que existen en tu types/cards.ts)
...
export function canTargetCreature(
  ...
  // (Los ataques ya se manejan en declareAttackCreature)
...
export function applyOnEnterEffects(
  ...
  // Aplicar efectos ON_ENTER de la carta si los tiene
  if (card.effects) {
    for (const eff of card.effects) {
      if (eff.timing === EffectTiming.ON_ENTER && effectConditionPasses(state, playerIndex, eff)) {
        // Crear hint para SELF si es necesario
...
// ✅ Función principal de Final Stand
...
function getUniqueAbilitiesFromGraveyard(player: PlayerState): string[] {
  ...
  for (const cardId of player.graveyard) {
    ...
    if (card && card.type === CardType.CREATURE && card.abilities) {
      card.abilities.forEach(ab => abilitiesSet.add(String(ab)))
    }
  }
  ...
  for (const effect of specimen.programmedEffects) {
    switch (effect) {
      case 'DAMAGE_3_ON_ENTER':
        // Aplica 3 de daño a un objetivo (puedes elegir el primero enemigo por defecto)
...
export function summonSpecimen(state: GameState, playerIndex: number): boolean {
  ...
  const cost = Math.min(10, 5 + 2 * specimenSummons)
  ...
  player.board.push({
    ...
    effects: [], // requerido por CreatureOnBoard
  })
  ...
  switch (player.classType) {
    case 'ABOMINACION': {
      ...
      for (const cardId of player.graveyard) {
        const card = getCardByIdGlobal(cardId)
        if (card && card.type === CardType.CREATURE && card.abilities) {
          card.abilities.forEach(ab => uniqueAbilities.add(String(ab)))
        }
      }
